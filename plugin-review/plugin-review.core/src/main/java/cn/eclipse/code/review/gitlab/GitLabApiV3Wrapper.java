...
public class GitLabApiV3Wrapper implements IGitLabApiWrapper {
	...
	public String getRevisionForLine(File file, String path, int lineNumber) {
		...
		for (String revision : config.getCommitSHA()) {
			if (patchPositionByFile.get(revision).entrySet().stream()
	...
	public GitlabIssue createOrUpdateIssue(Integer issueId, Integer assigneeId, Integer milestoneId,
		...
		if (issueId == null) {
			...
			return gitLabAPIV4.editIssue(projectId, issueId, assigneeId, milestoneId, labels, description, title,
					action);
		}
	}
	 ...
	 * 
	 * @see
	 * cn.eclipse.code.review.gitlab.IGitLabApiWrapper#createIssueNote(org.gitlab.
	 * api.models.GitlabIssue, java.lang.String)
	 */
	@Override
